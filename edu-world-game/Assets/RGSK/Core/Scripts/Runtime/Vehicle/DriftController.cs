...
namespace RGSK
{
    public class DriftController : RGSKEntityComponent
    {
        ...
        void EndDrift()
        {
            DebugTotalDriftPoints(); // Debug mỗi khi hoàn thành drift
            ResetCurrentValues();
            IsDrifting = false;
        ...
        bool Drifting()
        {
            ...
            var velo = Entity.Rigid.transform.InverseTransformDirection(Entity.Rigid.linearVelocity);
        ...
        // === Hàm mới để debug tổng điểm ===
        public void DebugTotalDriftPoints()
        {
            int totalDriftPoints = TotalPoints + TotalCurrentPoints;
            Debug.Log($"[DriftController] Tổng điểm drift hiện tại: {totalDriftPoints}");
        }
    }
}
