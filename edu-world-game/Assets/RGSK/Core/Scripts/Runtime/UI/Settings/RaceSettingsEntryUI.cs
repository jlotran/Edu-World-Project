...
namespace RGSK
{
    public class RaceSettingsEntryUI : GameSettingsEntryUI
    {
        ...
        {
            _session = session;
            _session.autoPopulateEntrantOptions.opponentClassOptions = OpponentClassOptions.SameAsPlayer;
        ...
        public void ToggleActive()
        {
            ...
            switch (type)
            {
                ...
                case RaceSettingUIType.StartingPosition:
                    {
                        gameObject.SetActive(!_session.IsSoloSession() && _session.raceType.minMaxCompetitorCount.y > 1 && RGSKCore.runtimeData.SelectedTrack.gridSlots > 1 && _session.autoPopulateEntrantOptions.opponentCount > 0);
                        break;
                    }
