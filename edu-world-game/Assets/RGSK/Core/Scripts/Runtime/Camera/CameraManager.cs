...
namespace RGSK
{
    public class CameraManager : Singleton<CameraManager>
    {
        ...
        IEnumerator Start()
        {
            ...
            yield return null;
            ChangeTarget(0);
            BindingCinemachineToTimeLine();
            RGSKEvents.OnTimelineToggle?.Invoke(false);
        }

        void BindingCinemachineToTimeLine()
        {
            _timeline = FindFirstObjectByType<Timeline>();
            _timeline?.BindingCinemachineBrain(cinemachineBrain);
            if (cinemachineVirtualCamera != null)
            {
                _timeline?.BindCinemachineVirtualCameraToShot(cinemachineVirtualCamera);
            }
        }
        ...
        void CreateCameras()
        {
            ...
            foreach (var p in _settings.perspectives)
            {
                if (p.virtualCamera != null)
                {
                    ...
                    if (cinemachineVirtualCamera == null)
                    {
                        cinemachineVirtualCamera = p.runtimeCamera;
                    }
                    else
                    {
                        continue;
        ...
        public void ChangePerspective(int direction)
        {
            ...
            {
                ToggleRouteCameras(false, CameraRouteType.End);
                _perspectiveIndex = 0;
                direction = 0;
            }
            ...
            if (_settings.cycleToRouteCameras || _inReplay)
            {
                ...
                {
                    ToggleRouteCameras(true, CameraRouteType.End);
                }
            }
        ...
        public void ChangeTarget(int direction)
        {
            ...
            if (RaceManager.Instance?.Session?.isFusionRacing ?? false && RaceManager.Instance?.CurrentState == RaceState.Racing)
            {
                return;
            }
        ...
        public void ToggleRouteCameras(bool toggle, CameraRouteType type)
        {
            if (_cameraRoute == null)
                return;

            _cameraRoute.Toggle_Camera(toggle, type);
