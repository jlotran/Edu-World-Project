...
namespace Fusion.Menu
{
    ...
    public interface IPhotonMenuConnection
  {
    ...
    /// <summary>
    /// Get available room from Photon namemserver. Must be implemented on SDK level.
    /// The list should be sorted by code.
    /// </summary>
    /// <returns>List of Photon regions, including the last ping, that are listed on the Photon dashboard for this AppId.</returns>
    Task<List<LobbyV3>> GetListLobby(IPhotonMenuConnectArgs connectArgs, GameRoomType roomType, LevelType level);
  }
}
...
namespace Fusion.Menu
{
    ...
    public abstract class PhotonMenuConnectionBehaviour : Fusion.Behaviour, IPhotonMenuConnection
  {
    ...
    public Task<List<LobbyV3>> GetListLobby(IPhotonMenuConnectArgs connectArgs, GameRoomType roomType, LevelType level = LevelType.Intern)
    {
      if (Connection == null)
      {
        Connection = Create();
      }
      return Connection.GetListLobby(connectArgs, roomType, level); 
    }
  }
}
