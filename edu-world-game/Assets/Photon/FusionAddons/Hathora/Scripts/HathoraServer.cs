...
namespace Fusion.Addons.Hathora
{
	public class HathoraServer : HathoraServerMgr
	{
		...
		private async Task<bool> Initialize(NetworkRunner runnerPrefab, HathoraServerContext serverContext, RoomWithoutAllocations room)
		{
			...
			string romconfigJson = room.RoomConfig;
			HathoraRoomConfig roomConfig = JsonUtility.FromJson<HathoraRoomConfig>(romconfigJson);

			int deathmatchSceneIndex = (int)roomConfig.roomType;
			Debug.Log("Start Scene "+deathmatchSceneIndex);
